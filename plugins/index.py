...
async def index_files(bot, query):
    ...
    if lock.locked():
        return await query.answer('Wait Until Previous Process Get Completed.', show_alert=True)
    msg = query.message

    await query.answer('Processing...⏳', show_alert=True)
    if int(from_user) not in ADMINS:
        await bot.send_message(int(from_user),
                               f'Your Submission For Indexing {chat} Has Been Accepted By Our Moderators And Files Will Be Added Soon.',
                               reply_to_message_id=int(lst_msg_id))
    await msg.edit(
        "Starting Indexing",
